...
form.addEventListener("submit", function(event) {
  ...
  const record = {
    ...
    late:
      typeof calculateLateFlag === "function"
        ? calculateLateFlag(
...
function calculateLateFlag(date, inValue) {
  if (!date || !inValue) return false;

  const settings =
    HRMS.getObject("salarySettings");

  if (!settings.shiftStart) return false;

  const shiftStart =
    minutesFromTime(settings.shiftStart);

  const actualIn =
    minutesFromTime(inValue);
  ...
  const allowedLate =
    Math.max(
      0,
      Number(settings.lateMinutes) || 0
    );

  return actualIn >
    shiftStart + allowedLate;
}

function getAutoDayStatus(dateValue) {
  ...
  if (
    settings.weeklyOff !== "" &&
    settings.weeklyOff !== undefined &&
    settings.weeklyOff !== null
  ) {
    const date =
...
/*
  Intercept attendance before saving so
  late + half-day rules are attached.
*/
...
form.addEventListener(
  ...
  function() {
    ...
    if (
      ...
      Number(workingHours.value || 0) > 0 &&
      Number(workingHours.value || 0) <
        Number(settings.halfDayHours)
    ) {
      attendanceStatus.value = "HD";
    }
...
/*
  After attendance save/update, attach
  calculated late flag to matching record.
*/
...
form.addEventListener(
  ...
  function() {
    ...
    setTimeout(() => {
      ...
      /*
        Form may already be reset, so this
        block only handles records when values
        are still available.
      */
